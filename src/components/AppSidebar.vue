...
const showHistory = computed(() => !collapsed.value && currentChats.value.length > 0 && (route.path === '/code' || route.path === '/mail'))
</script>
...
.label-fade-enter-from, .label-fade-leave-to {
  opacity: 0;
  transform: translateX(-6px);
}

.history-fade-enter-active, .history-fade-leave-active {
  transition: opacity 0.2s ease;
}
.history-fade-enter-from, .history-fade-leave-to {
  opacity: 0;
}
</style>